...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-surface">
    <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-line bg-[#15172a] p-5 text-white lg:flex lg:flex-col"><Logo /><div className="mt-8 rounded-xl bg-white/5 px-3 py-2"><p className="text-[10px] font-bold uppercase tracking-wider text-orange-300">Workspace</p><p className="mt-1 text-sm font-bold">Content Operations</p></div><nav className="mt-6 space-y-1">{items.map((item) => { const active = item.exact ? pathname === item.href : pathname.startsWith(item.href); return <Link key={item.href} href={item.href} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition", active ? "bg-orange-500 text-white" : "text-slate-400 hover:bg-white/5 hover:text-white")}><item.icon className="h-4 w-4" />{item.label}</Link>; })}</nav><div className="mt-auto border-t border-white/10 pt-4"><div className="flex items-center gap-3 px-2"><div className="grid h-9 w-9 place-items-center rounded-xl bg-orange-500 text-xs font-extrabold">AD</div><div className="min-w-0"><p className="truncate text-xs font-bold">{adminName}</p><p className="text-[10px] text-slate-500">{role === "SUPER_ADMIN" ? "Super Administrator" : "Administrator"}</p></div></div><button onClick={signOut} className="mt-3 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-400 hover:bg-white/5 hover:text-white"><LogOut className="h-4 w-4" /> Sign Out</button></div></aside>
