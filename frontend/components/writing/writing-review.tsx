...
function ReviewCard({ row, onGraded, onError }: {
  ...
  return <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_300px]">
    <div className="min-w-0">
      ...
      <p className="mb-1.5 mt-4 flex items-center gap-1.5 text-xs font-bold text-muted"><Highlighter className="h-3.5 w-3.5" /> Select any wrong fragment in the answer, then press &ldquo;Mark error&rdquo;.</p>
      ...
      {pending && <div className="mt-2.5 flex flex-wrap items-center gap-2 rounded-xl border border-red-300 bg-red-500/5 p-2.5">
        <span className="max-w-60 truncate rounded bg-red-500/10 px-2 py-1 text-xs font-bold text-red-600">&ldquo;{pending.text}&rdquo;</span>
      ...
      {sorted.length > 0 && <ol className="mt-3 space-y-1.5">
        {sorted.map((mark, index) => <li key={`${mark.start}-${index}`} className="flex items-start gap-2 text-xs leading-5">
          ...
          <span className="min-w-0 text-ink"><b className="text-red-600">&ldquo;{text.slice(mark.start, mark.end)}&rdquo;</b>{mark.comment ? <> — {mark.comment}</> : null}</span>
