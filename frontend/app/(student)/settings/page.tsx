...
export default function SettingsPage() {
  ...
  async function copyLink() {
    try {
      await navigator.clipboard.writeText(blockedLink);
      setCopied(true);
  ...
  async function linkTelegram() {
    setError(""); setBlockedLink(""); setLinking(true); setMessage("Opening Telegram. Tap “Share my phone number” in the bot, then return here.");
    try {
      const link = await api<{ token: string; deep_link: string }>("/auth/telegram/link/start", { method: "POST" });
      const telegramWindow = window.open(link.deep_link, "_blank", "noopener,noreferrer");
      if (!telegramWindow) { setMessage("Your browser blocked the Telegram window. Open the link below manually."); setBlockedLink(link.deep_link); }
      let attempts = 0;
      const poll = window.setInterval(async () => {
        attempts += 1;
        try {
          const result = await api<{ message: string }>(`/auth/telegram/link/complete?token=${encodeURIComponent(link.token)}`, { method: "POST" });
          ...
          if (reason instanceof ApiError && reason.status !== 409) {
            window.clearInterval(poll); setLinking(false); setError(reason.message);
          } else if (attempts >= 100) {
            window.clearInterval(poll); setLinking(false); setError("The Telegram link expired. Please try again.");
          }
        }
      }, 3000);
    } catch (reason) { setLinking(false); setError(reason instanceof Error ? reason.message : "Unable to start Telegram linking."); }
  }
  ...
  if (!me) return <div className="grid min-h-[70vh] place-items-center"><span className="h-10 w-10 animate-spin rounded-full border-4 border-orange-100 border-t-brand" /></div>;
  return <div className="mx-auto max-w-4xl px-4 py-8 sm:px-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-brand">Your account</p><h1 className="mt-2 text-3xl font-extrabold text-ink">Settings</h1><p className="mt-2 text-sm text-muted">Manage your profile, Telegram, password, and appearance.</p>
    ...
    {blockedLink && <div className="mt-3 rounded-xl border border-sky-300 bg-sky-500/10 p-3">
      <div className="flex items-center gap-2">
        <a href={blockedLink} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate text-sm font-semibold text-sky-700 underline">{blockedLink}</a>
