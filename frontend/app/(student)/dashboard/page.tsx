...
export default function Dashboard() {
  ...
  const completedByLevel = useMemo(() => completed.reduce<Record<string, number>>((result, item) => {
    result[item.level_slug] = (result[item.level_slug] ?? 0) + 1;
    return result;
  }, {}), [completed]);
  const displayedLevels: LevelCardData[] = catalog.map((row) => {
    const done = completedByLevel[row.slug] ?? 0;
  ...
  return <div className="mx-auto max-w-7xl px-4 py-7 sm:px-8 sm:py-9">
    ...
    <section className="mt-8 grid gap-4 sm:grid-cols-3">
      <StatCard label="Tests solved" value={String(completed.length)} hint={`${catalog.reduce((sum, item) => sum + item.published_tests, 0)} tests available`} icon={Target} tone="bg-indigo-500/10 text-indigo-500" />
    ...
    <section className="mt-10"><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-brand">RAR content</p><h2 className="mt-2 text-2xl font-extrabold text-ink">Choose your level</h2></div><span className="hidden text-sm text-muted sm:block">{catalog.reduce((sum, item) => sum + item.published_tests, 0)} imported tests</span></div><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{displayedLevels.map((level) => <LevelCard key={level.slug} level={level} />)}</div></section>
