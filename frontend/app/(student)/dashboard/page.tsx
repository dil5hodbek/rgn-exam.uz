...
export default function Dashboard() {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-7 sm:px-8 sm:py-9">
    ...
    <section className="mt-8 grid gap-4 sm:grid-cols-3">
      <StatCard label="Tests solved" value={String(solvedTests.length)} hint={`${catalog.reduce((sum, item) => sum + item.published_tests, 0)} tests available`} icon={Target} tone="bg-orange-500/10 text-orange-500" />
    ...
    {inProgress && <section className="relative mt-5 overflow-hidden rounded-3xl bg-[#171a38] p-6 text-white shadow-soft sm:p-8"><div className="relative z-10 flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-orange-300">Continue where you left off</p><h2 className="mt-3 text-2xl font-extrabold">{inProgress.title}</h2><p className="mt-2 text-sm text-orange-100/70">{inProgress.level} · {inProgress.exam_type}</p></div><Link href={`/dashboard/${inProgress.level_slug}/${inProgress.exam_type_slug}/${inProgress.test_variant_id}/attempt`}><Button className="bg-white text-[#171a38] shadow-none hover:bg-orange-50">Continue Test <ArrowRight className="h-4 w-4" /></Button></Link></div><div className="absolute -right-8 -top-24 h-72 w-72 rounded-full bg-orange-500/30 blur-3xl" /></section>}
