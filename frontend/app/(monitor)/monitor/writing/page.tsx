...
export default function WritingReview() {
  ...
  const counts = {
    ...
    ai_graded: rows.filter((row) => row.status === "ai_graded").length,
    teacher_graded: rows.filter((row) => row.status === "teacher_graded").length,
  };
  return <div className="mx-auto max-w-6xl p-4 sm:p-8">
    ...
    <p className="mt-2 text-sm text-muted">Writing is graded automatically by AI first. Check the AI's score, or grade by hand if it hasn't gotten to it yet.</p>
    <div className="mt-6 flex flex-wrap gap-2">
      {([
        ["all", `All (${counts.all})`],
        ["awaiting", `Awaiting review (${counts.awaiting})`],
        ["ai_graded", `AI graded (${counts.ai_graded})`],
        ["teacher_graded", `Teacher graded (${counts.teacher_graded})`],
      ] as const).map(([key, label]) => (
        <button key={key} onClick={() => setFilter(key)} className={`rounded-full px-3.5 py-1.5 text-xs font-bold transition ${filter === key ? "bg-brand text-white" : "bg-canvas text-muted hover:text-ink"}`}>{label}</button>
      ))}
    </div>
    ...
    <div className="mt-5 divide-y divide-line overflow-hidden rounded-2xl border border-line bg-canvas">
      {filtered.length
        ? filtered.map((row) => <ReviewCard key={row.id} row={row}
            onGraded={(id) => setRows((current) => current.map((item) => item.id === id ? { ...item, status: "teacher_graded" } : item))}
            onError={setError} />)
        : <p className="p-10 text-center text-sm text-muted">Nothing here.</p>}
    </div>
  </div>;
}
