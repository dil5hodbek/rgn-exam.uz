...
type Student = {
  ...
  is_active: boolean; total_attempts: number; average_percentage: number | null;
};
type StudentAttempt = {
...
function StudentAttempts({ studentId }: { studentId: string }) {
  ...
  useEffect(() => {
    api<{ attempts: StudentAttempt[] }>(`/teacher/students/${studentId}/attempts`)
...
export default function MonitorStudents() {
  ...
  async function load(value = "") { setRows(await api<Student[]>(`/teacher/students?search=${encodeURIComponent(value)}`)); }
  useEffect(() => { load(); }, []);
